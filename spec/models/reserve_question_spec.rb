...
RSpec.describe ReserveQuestion, type: :model do
  ...
  describe ".involving_related" do
    ...
    it "returns the question if the involvements match the project's involvements" do
      ...
      expect(ReserveQuestion.involving_related(project(:involves_amphibians))).to eq [amphibians_question]
      expect(ReserveQuestion.involving_related(project(:involves_fish))).to eq [fish_question]
      expect(ReserveQuestion.involving_related(project(:involves_birds))).to eq [birds_question]
      expect(ReserveQuestion.involving_related(project(:involves_plants_fungi_soil)))
        .to eq [plants_fungi_soil_question]
      expect(ReserveQuestion.involving_related(project(:involves_threatened_endangered_species)))
    ...
    it "returns a question that matches any one of the project's involvements" do
      ...
      expect(results).to match_array [mammals_question, fish_question]
      expect(results).not_to include birds_question
    end
    ...
    it "keeps every question for a project involving everything" do
      ...
      expect(ReserveQuestion.involving_related(project)).to match_array questions
    end
    ...
    it "qualifies its columns so it survives a join against projects" do
      ...
      results = ReserveQuestion
        .for_projects
        .involving_related(project)
        .with_answers_for_project(project)

      expect(results).to eq [fish_question]
    end
  end
end

# from path lib/tasks/utils/external_api.rake
...
namespace :external_api do
  ...
  task load_spdx_licenses: :environment do
    ...
    desc 'Populate the rors table from latest tmp/ror.json (single use) To force it to reprocess you can pass an argument `rails "external_api:ror_index[true]"` (Note the quotes)'
  # rubocop:enable Layout/LineLength
  task :sync_rors, [:force] => :environment do |_, args|
    p 'Processing the latest ROR registry from file tmp/ror.json and updating the rors table'

  ...
  def change
    ...
    reversible do |dir|
      dir.up do
        ...
        SQL
        execute <<~SQL
          UPDATE users SET gender_identity = 'Prefer not to state' WHERE gender_identity = '';
        SQL
        execute <<~SQL
          ALTER TABLE users MODIFY COLUMN gender_identity ENUM('Male','Female','Non-binary','Other','Prefer not to state');
        SQL
      ...
      dir.down do
        ...
        SQL
        execute <<~SQL
          UPDATE users SET gender_identity = '' WHERE (gender_identity = 'Other' OR gender_identity = 'Prefer not to state')
        SQL
        execute <<~SQL
          ALTER TABLE users MODIFY COLUMN gender_identity ENUM('Male','Female','Non-binary','');
        SQL
      end
    end
    ...
    rename_column :users, :BillingAddrLine1, :billing_address_line_1
    rename_column :users, :BillingAddrLine2, :billing_address_line_2

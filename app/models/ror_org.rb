...
class RorOrg < ApplicationRecord
  ...
  scope :by_acronym, lambda { |term|
    where(safe_json_lower_where_clause(table: 'ror_orgs', attribute: 'acronyms'),
          "%\"#{term}\"%")
  }

  scope :by_alias, lambda { |term|
    where(safe_json_lower_where_clause(table: 'ror_orgs', attribute: 'aliases'),
          "%\"#{term}\"%")
  }

  scope :by_name, lambda { |term|
    where('LOWER(ror_orgs.name) LIKE LOWER(?)', "%#{term}%")
  }

  scope :by_type, lambda { |term|
    where(safe_json_lower_where_clause(table: 'ror_orgs', attribute: 'types'),
          "%\"#{term}\"%")
  }

  scope :by_domain, lambda { |term|
    where('LOWER(ror_orgs.home_page) LIKE LOWER(?)', "%#{term}%")
  }

  # Get all of the RorOrg entries that have been connected to another object (e.g. Plan, User, etc.)
  scope :known, lambda {
    where.not(org_id: nil)
  }

  # Get all of the RorOrg entries that have NOT been connected to another object (e.g. Plan, User, etc.)
  ...
  # =================
  class << self
    # Get the RorOrg with the closest matching domain and no Org association

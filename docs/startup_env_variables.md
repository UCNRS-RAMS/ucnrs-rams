...
| PORT | container port | The port the application listens on (e.g. 3000) |
...
| RAILS_MIN_THREADS | sceptre config | The minimum number of threads for Rails (e.g. 5) per server |
...
| SMTP_HOST | ? | The SMTP host for sending emails (e.g. localhost)|
| SMTP_PORT | ? | The SMTP port for sending emails (e.g. 587) |
| SMTP_USERNAME | ? | The SMTP username for sending emails |
| SMTP_PASSWORD | ? | The SMTP password for sending emails |
| SMTP_DOMAIN | ? | The SMTP domain for sending emails (e.g. localhost) |

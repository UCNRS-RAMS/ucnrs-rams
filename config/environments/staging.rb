...
require Rails.root.join("lib/dev_server_structured_logging")
...
Rails.application.configure do
  ...
  # TODO: do we need to change the storage service to S3?
  ...
  # I think this may need to be true if the ALB terminates the SSL
  ...
  # TODO: configure for using a real smtp server
  config.action_mailer.delivery_method = :letter_opener_web
  config.action_mailer.perform_deliveries = true

  # External URL settings for links in emails/routes.
  app_host = ENV.fetch("MAILER_HOST", ENV.fetch("HOST", "rams-stg.cdlib.net"))
  ...
  # JSON formatter for non-request logs and parsed request logs.
  # Implementation lives in lib/ for reuse and cleaner environment config.
  config.log_formatter = DevServerStructuredLogging.json_formatter

...
note that using bin/dev will now allow access to debugging breakpoints such as byebug

One solution is to comment out the `web: env . . .` line in `Procfile.dev`, add the `byebug` breakpoint and then run the web
server separately with `bin/rails s` (you will still need to run `bin/dev` for the css and js processes).  When the breakpoint happens you can then interact with the debugger in the web application this way.
```
